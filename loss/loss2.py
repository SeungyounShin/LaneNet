...
def mae(y_true, y_pred):
    ...
    return torch.mean(torch.abs(y_true[over_threshold]-y_pred[over_threshold]))

def fscore(y_true, y_pred):
    remove_NAs = y_true >= 0
    true_shape = (y_true[remove_NAs] >= 0.1).shape
    pred_shape = (y_pred[remove_NAs] >= 0.1).shape
    y_true = torch.where(y_true[remove_NAs] >= 0.1, torch.ones(true_shape).view(-1), torch.zeros(true_shape).view(-1))
    y_pred = torch.where(y_pred[remove_NAs] >= 0.1, torch.ones(pred_shape).view(-1), torch.zeros(pred_shape).view(-1))
    return f1_score(y_true, y_pred)

def maeOverFscore(y_true, y_pred):
    return mae(y_true, y_pred) / (fscore(y_true, y_pred) + 1e-04)
...
class FocalTversky_loss(nn.Module):
    ...
    """
    def __init__(self, apply_nonlin=None, batch_dice=False, do_bg=True, smooth=1.,square=False, gamma=2.1):
        super(FocalTversky_loss, self).__init__()
        self.gamma = gamma
        self.tversky = TverskyLoss(apply_nonlin=None, batch_dice=False, do_bg=True, smooth=1.,square=False)
